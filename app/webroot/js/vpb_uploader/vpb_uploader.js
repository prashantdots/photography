		...
		if(this.vpb_files.length > 0)
		{
 			...
 			for(var i = 0; i<this.vpb_files.length; i++)
			{
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					{
						vpb_file_icon = '<img src="'+baseUrl+'images/images_file.gif" align="absmiddle" border="0" alt="" />';
		...
		if(typeof file[file_counter] != undefined && file[file_counter] != '')
		{
			...
			else
			{
				...
				$.ajax({
					...
					beforeSend: function() 
					{
						$("#uploading_"+ids).html('<div align="left"><img src="'+baseUrl+'images/loadings.gif" width="80" align="absmiddle" title="Upload...."/></div>');
